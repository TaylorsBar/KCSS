...
import { obdService } from '../services/obdService.ts';
  ...
  tick() {
      ...
      if (currentGpsData?.speed != null && currentGpsData.speed > 0.5) {
        ...
        switch (this.simState.vehicleState) {
          case 1: if (rpm > 4500 && gear < 6) { gear++; rpm *= 0.6; } rpm += (this.RPM_MAX / ((gear || 1) * 15)) * (1 - rpm/this.RPM_MAX) + Math.random() * 50; break;
        ...
        if (gear > 0 && this.GEAR_RATIOS[gear]) {
            speed = (rpm / (this.GEAR_RATIOS[gear] * 300)) * (1 - (1/gear)) * 10;
        } else {
            speed *= 0.98; // Speed decays when in neutral
        }
