...
import { getTuningSuggestion, analyzeTuneSafety, getTuningChatResponse } from '../services/geminiService.ts';
import { TuningSuggestion, ChatMessage, AuditEvent, HederaEventType } from '../types.ts';
import TuningSlider from '../components/tuning/TuningSlider.tsx';
import TuningMap from '../components/tuning/TuningMap.tsx';
import SparklesIcon from '../components/icons/SparklesIcon.tsx';
import ReactMarkdown from 'react-markdown';
import { useTrainingStore } from '../hooks/useTrainingStore.ts';
import FeatureLock from '../components/DataBar.tsx';
...
const generateDefaultMap = (baseValue: number): number[][] =>
  Array(LOAD_AXIS.length)
    .fill(0)
    .map(() => Array(RPM_AXIS.length).fill(baseValue));
...
const TuningPage: React.FC = () => {
  ...
  const handleMapChange = (map: 'ignitionTiming' | 'boostPressure', row: number, col: number, value: number) => {
    if (map === 'boostPressure') {
      ...
      const newMap = currentTune.boostPressure.map((r) => [...r]);
      newMap[row][col] = parseFloat(baseValue.toFixed(2));
      setCurrentTune((prev) => ({ ...prev, boostPressure: newMap }));
    } else {
      const newMap = currentTune.ignitionTiming.map((r) => [...r]);
  ...
  const applySuggestion = (suggestion: TuningSuggestion, goal: string) => {
    ...
    setAiChat((prev) => [...prev, { id: Date.now().toString(), sender: 'ai', text }]);
  ...
  const handleGetSuggestion = async (goal: string) => {
    setAiIsLoading(true);
    setAiChat((prev) => [...prev, { id: Date.now().toString(), sender: 'user', text: `Generate a tune for: ${goal}` }]);
    ...
    try {
      ...
      setAiChat((prev) => [...prev, { id: Date.now().toString(), sender: 'ai', text: `Sorry, I couldn't generate a tune. Error: ${error}` }]);
    } finally {
  ...
  const handleSafetyCheck = async () => {
    setAiIsLoading(true);
    setSafetyReport(null);
    setAiChat((prev) => [...prev, { id: Date.now().toString(), sender: 'user', text: 'Analyze the safety of my current tune.' }]);
    ...
    try {
      ...
      setAiChat((prev) => [...prev, { id: (Date.now() + 1).toString(), sender: 'ai', text: safetyMessage }]);
    } catch (e) {
      const error = e instanceof Error ? e.message : 'An unknown error occurred.';
      setAiChat((prev) => [...prev, { id: (Date.now() + 1).toString(), sender: 'ai', text: `Sorry, I couldn't analyze the tune. Error: ${error}` }]);
    } finally {
      setAiIsLoading(false);
    }
  };
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAiChat((prev) => [...prev, { id: (Date.now() + 1).toString(), sender: 'ai', text: response }]);
    } catch (e) {
      const error = e instanceof Error ? e.message : 'An unknown error occurred.';
      setAiChat((prev) => [...prev, { id: (Date.now() + 1).toString(), sender: 'ai', text: `Sorry, I couldn't get a response. Error: ${error}` }]);
    } finally {
      setAiIsLoading(false);
    }
  };
        ...
        <div className="flex-shrink-0 space-y-2 pt-2 border-t border-[var(--theme-accent-primary)]/30">
          <form onSubmit={handleChatSubmit} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-base-800 border border-base-700 rounded-md px-3 py-2 text-sm text-gray-200"
              disabled={aiIsLoading}
            />
            <button type="submit" disabled={aiIsLoading} className="btn btn-primary">
              Send
            </button>
          </form>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <button onClick={() => handleGetSuggestion('Max Performance')} disabled={aiIsLoading} className="btn btn-secondary flex items-center justify-center gap-1">
              <SparklesIcon className="w-4 h-4 text-brand-pink" /> Track Day
            </button>
            <button onClick={() => handleGetSuggestion('Fuel Economy')} disabled={aiIsLoading} className="btn btn-secondary flex items-center justify-center gap-1">
              <SparklesIcon className="w-4 h-4 text-brand-green" /> Eco Tune
            </button>
            <button onClick={() => handleGetSuggestion('Daily Driving')} disabled={aiIsLoading} className="btn btn-secondary flex items-center justify-center gap-1">
              <SparklesIcon className="w-4 h-4 text-[var(--theme-accent-primary)]" /> Daily
            </button>
            <button onClick={handleSafetyCheck} disabled={aiIsLoading} className="btn btn-warning font-semibold">
              Safety Check
            </button>
          </div>
        </div>
      ...
      <div className="lg:col-span-2 bg-black p-4 rounded-lg border border-[var(--theme-accent-primary)]/30 shadow-lg flex flex-col">
        <div className="flex justify-between items-center border-b border-[var(--theme-accent-primary)]/30 pb-2 mb-2">
          <div className="flex gap-2">
            {(['ignition', 'boost', 'fuel'] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-4 py-2 text-sm font-semibold rounded-lg border-2 transition-all capitalize ${
                  activeTab === tab
                    ? 'border-[var(--theme-accent-primary)] text-white shadow-glow-theme bg-[var(--theme-accent-primary)]/20'
                    : 'border-base-700 bg-transparent text-gray-400 hover:border-[var(--theme-accent-primary)]/50'
                }`}
              >
                {tab}
              </button>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                setCurrentTune(DEFAULT_TUNE);
                setBoostPressureOffset(0);
              }}
        ...
        <div className="flex-grow mt-2 overflow-y-auto">
          {activeTab === 'ignition' && (
            <TuningMap
              title="Ignition Timing (deg BTDC)"
              data={currentTune.ignitionTiming}
              xAxisLabels={RPM_AXIS}
              yAxisLabels={LOAD_AXIS}
              onChange={(r, c, v) => handleMapChange('ignitionTiming', r, c, v)}
            />
          )}
          {activeTab === 'boost' && (
            <div>
              ...
              <div className="p-4 border-t border-[var(--theme-accent-primary)]/30 mt-4">
                <TuningSlider
                  ...
                  max={0.5}
                  step={0.05}
                  onChange={setBoostPressureOffset}
                />
              </div>
            </div>
          )}
          {activeTab === 'fuel' && (
            <div className="p-8">
              ...
              <TuningSlider label="Fuel Trim" unit="%" value={currentTune.fuelMap} min={-10} max={10} step={0.5} onChange={(v) => setCurrentTune((p) => ({ ...p, fuelMap: v }))} />
            </div>
          )}
          {safetyReport && (
            <div className="mt-4 p-3 rounded-md bg-base-800 text-sm">
              Safety score: <span className="font-mono text-[var(--theme-accent-primary)]">{safetyReport.score}/100</span>
            </div>
          )}
        </div>
      </div>
