...
import {
  ...
  getCrewChiefResponse,
  getRouteScoutResponse,
  getComponentHealthAnalysis,
...
const ToggleSwitch: React.FC<{ checked: boolean; onChange: (checked: boolean) => void }> = ({ checked, onChange }) => (
  <button
    ...
    className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none ${
      checked ? 'bg-[var(--theme-accent-primary)]' : 'bg-base-700'
    }`}
    role="switch"
  ...
  <button
    ...
    className={`flex-1 flex flex-col items-center justify-center gap-1 p-3 text-sm font-semibold transition-colors ${
      isActive
        ...
        : 'bg-base-800/30 text-gray-400 hover:bg-base-800/50'
    }`}
  >
    {icon}
    <span>{label}</span>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-1 bg-black p-6 rounded-lg border border-[var(--theme-accent-primary)]/30 shadow-lg space-y-4">
        <div className="flex items-center justify-between p-3 bg-base-800/50 rounded-md">
          <label className="font-semibold text-white">Proactive Monitoring</label>
        ...
        <div className="text-center p-4 bg-base-900/50 rounded-md">
          ...
          <p className="text-xs text-gray-500 mt-2">
            Last analysis: {lastAnalysisTimestamp ? lastAnalysisTimestamp.toLocaleTimeString() : 'Never'}
          </p>
        </div>
...
const AgentChatInterface: React.FC<{ agent: 'crew-chief' | 'route-scout' }> = ({ agent }) => {
  ...
  const agentConfig = {
    'crew-chief': {
      ...
      initialMessage: "I'm your Crew Chief. Tell me what parts you're looking for.",
    ...
    'route-scout': {
      ...
      initialMessage: "I'm your Route Scout. Ask me to find interesting driving routes.",
      handler: (query: string) =>
        getRouteScoutResponse(query, { latitude: latestData.latitude, longitude: latestData.longitude }),
    },
  };
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), text: aiResponse.text, sender: 'ai', chunks: aiResponse.chunks },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), text: "Sorry, I couldn't get a response.", sender: 'ai' },
      ]);
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="space-y-4">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
              {msg.sender === 'ai' && (
                <div className="w-8 h-8 rounded-full bg-[var(--theme-accent-primary)] flex-shrink-0 mt-1 shadow-glow-theme" />
              )}
      ...
      <div className="p-4 border-t border-[var(--theme-accent-primary)]/30">
        <form onSubmit={handleSend} className="flex items-center gap-3">
          <input
            ...
            className="flex-1 bg-base-800 border border-base-700 rounded-md px-4 py-2 text-gray-200"
            disabled={isLoading}
          />
