...
import { ConnectionStatus, DTCInfo, GroundedResponse, SensorDataPoint } from '../types';
...
import VideoCameraIcon from '../components/icons/TuneIcon'; // Repurposed for VideoCameraIcon
...
const DiagnosticsDeck: React.FC = () => {
    ...
    const chartData = useMemo(() => {
        return vehicleDataHistory.map(d => ({
            ...d,
            afr: 14.7 + (d.o2SensorVoltage - 0.45) * 5
        }));
    }, [vehicleDataHistory]);

    return (
        <div className="p-4 h-full overflow-y-auto">
            <h2 className="text-xl font-bold text-gray-100 font-display text-center mb-4">Live Data Deck & Telemetry</h2>
            ...
            <div className="grid grid-cols-1 gap-6">
                 <SensorChart 
                    data={chartData as SensorDataPoint[]}
                    lines={[
                        { dataKey: 'afr', name: 'Air/Fuel Ratio', stroke: 'var(--theme-accent-primary)' },
                    ]}
                ...
                <SensorChart 
                    data={chartData}
                    lines={[
                        { dataKey: 'fuelPressure', name: 'Fuel Pressure', stroke: 'var(--theme-accent-green)' },
                    ]}
                    title="Fuel Pressure (bar)"
                />
                <SensorChart 
                    data={chartData}
                    lines={[
                        { dataKey: 'egt', name: 'Exhaust Gas Temp', stroke: 'var(--theme-accent-red)' },
                    ]}
                    title="Exhaust Gas Temperature (°C)"
                />
            </div>
        </div>
    )
...
const VideoAnalyst: React.FC = () => {
    ...
    const extractFramesFromVideo = (file: File): Promise<{data: string, mimeType: string}[]> => {
        return new Promise((resolve, reject) => {
            ...
            videoEl.onloadedmetadata = () => {
                ...
                const interval = duration > MAX_FRAMES ? duration / MAX_FRAMES : 1;
                let currentTime = 0;
                ...
                videoEl.onseeked = () => {
                    ...
                    frames.push({ data: base64String, mimeType: 'image/jpeg' });
                    framesCaptured++;
                    currentTime += interval;
                    captureFrame();
                };
                
                videoEl.onerror = (e) => reject(new Error('Error loading video file for frame extraction.'));
                
                // Play and immediately pause to enable seeking on some browsers
                videoEl.play().then(() => {
                    videoEl.pause();
                    captureFrame();
                }).catch(e => reject(e));
    ...
    const handleAnalyze = async () => {
        ...
        setError('');
        try {
            const frames = await extractFramesFromVideo(video.file);
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full p-4">
            <div className="flex flex-col gap-4">
                <h2 className="text-xl font-bold text-gray-100 font-display text-center">Video Analyst</h2>
                {!video ? (
                    <div 
                        onDrop={dropHandler} onDragOver={dragOverHandler} onDragEnter={() => setIsDragging(true)} onDragLeave={() => setIsDragging(false)}
                        onClick={() => fileInputRef.current?.click()}
                        className={`flex-grow flex flex-col items-center justify-center border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragging ? 'border-[var(--theme-accent-primary)] bg-[var(--theme-accent-primary)]/10' : 'border-base-700 hover:border-gray-500'}`}
                    >
                        <VideoCameraIcon className="w-12 h-12 text-gray-500 mb-2" />
                        <p className="text-gray-400">Drag & drop a video here</p>
                        <p className="text-sm text-gray-500">or click to select a file</p>
                        <input ref={fileInputRef} type="file" accept="video/*" onChange={(e) => handleFileChange(e.target.files)} className="hidden" />
                    </div>
                ) : (
                    <div className="flex-grow relative rounded-lg overflow-hidden border-2 border-base-700">
                        <video src={video.previewUrl} controls className="w-full h-full object-contain" />
                    </div>
                )}
            </div>
            <div className="flex flex-col gap-4">
                <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)}
                    placeholder={video ? "Ask a question about the video...\ne.g., 'What is the noise at 10 seconds?' or 'Is this engine running smoothly?'" : "Upload a video to ask a question."}
                    disabled={!video || isLoading}
                    className="w-full flex-grow bg-base-800 border border-base-700 rounded-md p-3 text-gray-200 placeholder-gray-500 resize-none focus:outline-none focus:ring-2 focus:ring-[var(--theme-accent-primary)]"
                />
                <div className="flex gap-2">
                    <button onClick={handleAnalyze} disabled={!video || !prompt.trim() || isLoading} className="btn btn-primary flex-grow flex items-center justify-center gap-2">
                        <SparklesIcon className="w-5 h-5" />
                ...
                <div className="flex-grow bg-base-900/50 rounded-lg border border-base-700 p-3 overflow-y-auto">
                    {isLoading && (
                        <div className="flex flex-col items-center justify-center h-full">
                            <div className="loader ease-linear rounded-full border-4 border-t-4 border-gray-200 h-10 w-10 animate-spin border-t-[var(--theme-accent-primary)]"></div>
                            <p className="text-sm text-gray-400 mt-4">Analyzing video... this may take a moment.</p>
                        </div>
                    )}
                    {error && <p className="text-red-400">{error}</p>}
                    {analysis && (
                        <div className="prose prose-sm prose-invert max-w-none"><ReactMarkdown>{analysis}</ReactMarkdown></div>
                    )}
                </div>
            </div>
        </div>
        ...
        <div className="p-4 h-full flex flex-col">
            ...
            <div className="flex-shrink-0 border-b border-[var(--glass-border)] mt-4 overflow-x-auto">
                <div className="flex">
                    ...
                    <TabButton label="Live Data Deck" isActive={activeTab === 'deck'} onClick={() => setActiveTab('deck')} />
                    ...
                    <TabButton label="Video Analyst" icon={<VideoCameraIcon className="w-5 h-5"/>} isActive={activeTab === 'video'} onClick={() => setActiveTab('video')} />
                </div>
            </div>
