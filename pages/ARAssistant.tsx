...
import { getVoiceCommandIntent } from '../services/geminiService';
        ...
        switch (result.intent) {
            case IntentAction.ShowComponent:
                if (result.component && MOCK_HOTSPOTS.find(h => h.id === result.component)) {
                    ...
                    const componentData = MOCK_HOTSPOTS.find(h => h.id === result.component);
        ...
        <div className="flex flex-col lg:flex-row gap-6 h-full">
            ...
            <div className="w-full lg:w-2/3 bg-black rounded-lg border border-brand-cyan/30 shadow-lg flex flex-col items-center justify-center p-4 relative overflow-hidden">
                ...
                <div className="relative w-full max-w-3xl aspect-video">
                    <img src="https://storage.googleapis.com/fpl-assets/ar-engine-wireframe.svg" alt="Engine Wireframe" className="w-full h-full object-contain" />
                    <svg className="absolute top-0 left-0 w-full h-full" viewBox="0 0 1280 720">
                        {MOCK_HOTSPOTS.map(hotspot => (
                            <g key={hotspot.id} className={`transition-opacity duration-300 ${highlightedComponent === hotspot.id ? 'opacity-100' : 'opacity-0'}`}>
                                <circle 
                                    cx={hotspot.cx} 
                                    cy={hotspot.cy} 
                                    r="20" 
                                ...
                                >
                                 ...
                                 <circle 
                                    ...
                                    strokeWidth="2"
                                />
                                <text x={hotspot.cx} y={hotspot.cy} dy="-30" textAnchor="middle" className="fill-white font-sans font-bold text-lg drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]">
                                    {hotspot.name}
                                </text>
                            </g>
                        ))}
